...
/**
 * Send msg to reg_id, topic, aliases, all
 *
 * @api private
 */
MiPush.prototype.send_msg = function(msg, option, url){
  ...
  // Set notify_effect
  if(option.hasOwnProperty('notify_effect')){
    msg['extra.notify_effect'] = option.notify_effect;
  }
  ...
  // Custom app_version for android
  if(option.hasOwnProperty('extra.app_version')){
    msg['extra.app_version'] = option['extra.app_version'];
  }
  ...
  if(option['extra.type']){
    msg['extra.type'] = option['extra.type'];
  }

  if(option['extra.source']){
    msg['extra.source'] = option['extra.source'];
  }

  if(option['extra.from']){
    msg['extra.from'] = option['extra.from'];
  }
